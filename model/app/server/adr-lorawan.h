...
class AdrLorawan : public NetworkControllerComponent
{
  ...
  protected:
    ...
    // New methods for vectors oprtns - @geraldosarmento
    double VecMax(const std::vector<double>& values);
    double VecAVG(const std::vector<double>& values);
    double VecSD(const std::vector<double>& values, double mean);
    std::vector<double> GetRealisticSNR(double mean, int historyRange);  
    ...
    // Factor of SNR variation
    double m_rawSNRFactor;
};
} // namespace lorawan
} // namespace ns3
